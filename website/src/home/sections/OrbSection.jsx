    ...
    function draw(p) {
      ...
      for (let ring = 1; ring <= 4; ring++) {
        ...
        for (let i = 0; i < RADAR_N; i++) {
          const a = (i / RADAR_N) * Math.PI * 2;
          const pt = toXY(a, r, cx, cy);
      ...
      const pts = scores.map((s, i) => {
        const a = (i / RADAR_N) * Math.PI * 2;
        return toXY(a, (s / RADAR_MAX) * maxR * p, cx, cy);
      });
      ctx.beginPath();
      ...
      for (let i = 0; i < RADAR_N; i++) {
        const a = (i / RADAR_N) * Math.PI * 2;
        const pt = toXY(a, labelR, cx, cy);
        const lines = RADAR_AXES[i].split('\n');
  ...
  return (
    ...
    >
      <div style={{ marginBottom: 12 }}>
        <RepRadarCanvas scores={rep.scores} strokeColor={rep.strokeColor} size={180} />
      </div>
...
function OrbCanvas({ size }) {
  ...
  useEffect(() => {
    ...
    function draw(ts) {
      ...
      ctx.strokeStyle = `rgba(${cr},${cg},${cb},${0.18 - 0.1 * pulseT})`;
      ctx.lineWidth = 1;
      ctx.stroke();
      ctx.restore();
      ctx.save();
      ...
      ctx.strokeStyle = `rgba(${cr},${cg},${cb},${0.1 - 0.07 * pulseT2})`;
      ctx.lineWidth = 1;
      ctx.stroke();
      ctx.restore();
  ...
  return (
    <canvas
      ...
      style={{
        display: 'block',
        filter:
          'drop-shadow(0 0 48px rgba(242,107,69,0.32)) drop-shadow(0 0 120px rgba(75,163,227,0.18))',
      }}
    />
  );
}
...
const ANSWERS = {
  'Why are we losing deals to Salesforce in Q4?': {
    answer:
      '4 deals lost to Salesforce in the last 60 days. In 3 of those, implementation timeline came up before value was established. Your top performers address timeline at minute 8. Your team average is minute 23.',
    source: 'Based on 847 conversations · Updated 2 hours ago',
  },
  'Which rep struggles most on pricing?': {
    answer:
      'Daan loses frame on pricing in 68% of his deals - the highest on the team. The pattern: he drops price before the prospect asks. Top performers wait for explicit pushback.',
    source: 'Based on 312 rep interactions · Updated 4 hours ago',
  },
  'What do our best clients have in common?': {
    answer:
      'Your top 20% of clients by ACV share 3 signals: CTO involved in first 2 calls, implementation timeline discussed in meeting 1, and a prior failed vendor mentioned in discovery.',
    source: 'Based on 1,204 client interactions · Updated 1 day ago',
  },
  'What did prospects say about onboarding last quarter?': {
    answer:
      '"Implementation feels risky" appeared in 34 conversations last quarter. 8 of those became lost deals. It comes up when deployment timeline isn\'t addressed in the first meeting. Your top performers raise it proactively at minute 12.',
    source: 'Based on 312 conversations · Updated 5 hours ago',
  },
  'Where does the team stall most often?': {
    answer:
      '67% of stalled deals share one signal: no follow-up email within 24 hours of the call. Your top performers send a recap within 2 hours. Team average: 31 hours.',
    source: 'Based on 203 stalled deals · Updated 3 days ago',
  },
  "What changed in Jana's last three calls?": {
    answer:
      'Tone shifted from evaluative to cautious. Budget mentioned 4 times across the last 3 calls vs 0 in her first 2. Salesforce came up in call 3 for the first time. Risk: deal may be stalling at procurement.',
...
function QuestionCard({ question, onSelect, answered, idx, isSelected }) {
  ...
  return (
    <motion.button
      ...
      onClick={(e) => {
        e.stopPropagation();
        if (!answered && !isSelected) onSelect(question);
      }}
...
export default function OrbSection() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) setOrbVisible(true);
      },
      { threshold: 0.15 }
    );
    observer.observe(el);
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: 'center', marginBottom: 52, maxWidth: 720 }}>
        ...
        >
          Not a dashboard. A plain-language interface to everything your team has ever said. Click
          any question.
        </div>
      </div>
      ...
      >
        {orbVisible &&
          QUESTIONS.map((q, i) => (
            ...
            >
              {answeredQ.has(q) && (
                <span style={{ color: 'var(--coral)', marginRight: 8, fontSize: 11 }}>✓</span>
              )}
              {q}
            </motion.button>
          ))}
