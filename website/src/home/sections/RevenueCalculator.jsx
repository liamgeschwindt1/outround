...
const POST_CALL_RECOVERY = 0.9; // 90% of post-call logging
const PRE_CALL_RECOVERY = 0.7; // 70% of pre-call research
const MIN_RECOVERED_PER_CALL =
  POST_CALL_LOG_MIN * POST_CALL_RECOVERY + PRE_CALL_RESEARCH_MIN * PRE_CALL_RECOVERY; // 54

// Conservative discount on recovered time (not all saved time becomes selling time)
const EFFICIENCY_DISCOUNT = 0.4;
...
const DISPLACED_CALLS_PER_REP_WEEK =
  ((MIN_LOST_PER_CALL * CALLS_PER_REP_DAY) / 60 / CALL_DURATION_MIN) * 60;
...
function calc(dealLabel, cycleLabel, numReps) {
  ...
  return {
    ...
    callsPerCycle,
    displacedCallsCycle,
    displacedCallsPerRepWeek: DISPLACED_CALLS_PER_REP_WEEK,
    recoveredCallsCycle,
    recoveredCallsPerRep: RECOVERED_CALLS_PER_REP,
    fullPotential,
    lostPipeline,
    recoveredPipeline,
    annualPotential,
...
export default function RevenueCalculator() {
  ...
  useEffect(() => {
    ...
    // Defer resets via rAF so they're not synchronous in the effect.
    // The animation sequence restarts when the user reaches the results step.
    const raf1 = requestAnimationFrame(() => setShowBigNum(false));
    const raf2 = requestAnimationFrame(() => setShowBarStats(false));
    const raf3 = requestAnimationFrame(() => setBarTriggered(false));
    const raf4 = requestAnimationFrame(() => setShowAfterStats(false));
  ...
  const proofRows = c
    ? [
        ...
        },
        {
          ...
          source:
            'Bridge Group SDR Metrics Report \u2014 8 connected calls/rep/day benchmark; Belkins B2B Outbound Benchmarks 2024',
        },
      ]
    : [];
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                        {DEAL_OPTIONS.map((opt) => (
                          <ChoiceButton key={opt} label={opt} onClick={() => selectDeal(opt)} />
                        ))}
                      </div>
                    </motion.div>
                  )}
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                        {CYCLE_OPTIONS.map((opt) => (
                          <ChoiceButton key={opt} label={opt} onClick={() => selectCycle(opt)} />
                        ))}
                      </div>
                    </motion.div>
                  )}
                        ...
                        <AnimatePresence>
                          {showBigNum && (
                            ...
                            >
                              ...
                              >
                                Where a rep&rsquo;s day goes &middot; {c.numReps} rep
                                {c.numReps !== 1 ? 's' : ''}
                              </div>
                                    ...
                                    >
                                      <div>
                                        ...
                                        <div
                                          style={{
                                            fontFamily: 'var(--font-mono)',
                                            fontSize: 10,
                                            color: 'rgba(242,107,69,0.4)',
                                          }}
                                        >
                                          {c.numReps} rep{c.numReps !== 1 ? 's' : ''} &middot;{' '}
                                          {c.cycleLabel.toLowerCase()} &middot; 3% close
                                        </div>
                                      </div>
                                      <div
                                        style={{
                                      ...
                                      >
                                        <CountingNumber
                                          ...
                                          target={
                                            barTriggered
                                              ? (c.annualLost - c.annualRecovered) / 12
                                              : c.annualLost / 12
                                          }
                              ...
                              >
                                {[
                                  ...
                                  {
                                    label: `${c.numReps} rep${c.numReps > 1 ? 's' : ''}`,
                                    step: 'team',
                                  },
                                ].map((chip) => (
                                  <button
                                ...
                                <div style={{ marginBottom: 20 }}>
                                  {!barTriggered ? (
                                    <motion.button
                                      whileHover={{
                                        scale: 1.02,
                                        boxShadow: '0 0 40px rgba(242,107,69,0.4)',
                                      }}
                                      whileTap={{ scale: 0.98 }}
                                      onClick={triggerBar}
                                      style={{
                                  ...
                                  {showMethod && (
                                    ...
                                    >
                                      <div style={{ paddingTop: 14 }}>
                                        ...
                                        >
                                          Figures shown are pipeline value (deals \u00d7 ACV), not
                                          closed revenue. Assumes 8 connected calls per rep per day
                                          at the SDR benchmark and a 5-day working week.
                                        </div>
                                      </div>
                                    </motion.div>
                                  )}
                              ...
                              {showBarStats && (
                                ...
                                >
                                  ...
                                  >
                                    Every call resets memory. Every rep rebuilds context. That is
                                  ...
                                  <button
                                    ...
                                    onMouseEnter={(e) => {
                                      e.currentTarget.style.borderColor = 'rgba(242,107,69,0.8)';
                                      e.currentTarget.style.background = 'rgba(242,107,69,0.06)';
                                    }}
                                    onMouseLeave={(e) => {
                                      e.currentTarget.style.borderColor = 'rgba(242,107,69,0.45)';
