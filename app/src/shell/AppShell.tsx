...
function ProfilePopup({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      {[{ label: 'Account settings', to: '/settings' }].map(({ label, to }) => (
        <button
          key={to}
          onClick={() => {
            nav(to);
...
function Sidebar() {
  ...
  return (
    ...
    >
      {/* Logo */}
      <div
        style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20, paddingLeft: 4 }}
      >
        <div
          style={{ width: 22, height: 22, borderRadius: 6, background: T.grad, flexShrink: 0 }}
        />
        <span
          style={{
