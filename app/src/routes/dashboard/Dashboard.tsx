...
function NextMeetingCard({ meeting }: { meeting: UpcomingMeeting }) {
  ...
  if (!isPast && mins < 60) timeLabel = `In ${String(mins)}m`;
  else if (!isPast && mins < 1440) timeLabel = `In ${String(Math.round(mins / 60))}h`;
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <div style={{ fontSize: 17, fontWeight: 700, color: T.t1, lineHeight: 1.3 }}>
            {meeting.title}
          </div>
        ...
        {meeting.bot && (
          <span
            style={{
              ...
              background:
                meeting.bot.status === 'done' ? 'rgba(22,163,74,0.1)' : 'rgba(61,159,212,0.1)',
        ...
        >
          <div
            style={{
              ...
              fontSize: 13,
              fontWeight: 700,
              color: s.score != null ? scoreColor(s.score) : T.t3,
              flexShrink: 0,
              fontVariantNumeric: 'tabular-nums',
            }}
          >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 11, color: T.t3, fontFamily: T.mono }}>
              ...
              {s.duration_seconds && ` · ${String(Math.round(s.duration_seconds / 60))}m`}
            </div>
          </div>
...
export default function Dashboard() {
  ...
  const now = new Date();
  const nextMeeting: UpcomingMeeting | null =
    allMeetings.find((m) => new Date(m.starts_at) >= now) ??
    (allMeetings.length > 0 ? allMeetings[allMeetings.length - 1] : null);
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      >
        ...
        <StatCard
          label="Best score"
          value={s?.best_score != null ? String(s.best_score) : '—'}
          sub="/100"
