...
export default function MeetingBotPage() {
  ...
  const resolvedAutoJoin = autoJoin !== null ? autoJoin : (savedSettings?.auto_join ?? true);
  const resolvedJoinType = joinType !== (savedSettings?.join_type ?? 'all') ? joinType : (savedSettings?.join_type ?? 'all');

  const saveSettings = async () => {
    setSettingsSaving(true);
    setSettingsOk(false);
    try {
      await api.put('/api/bots/settings', { auto_join: resolvedAutoJoin, join_type: resolvedJoinType });
      setSettingsOk(true);
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      {/* Auto-join settings */}
      <div style={{ background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: R.xl, padding: 20, marginBottom: 20 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: T.t1, marginBottom: 16 }}>Auto-join settings</div>

        {/* Auto-join toggle */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 13, color: T.t1, fontWeight: 500 }}>Automatically join calendar meetings</div>
            <div style={{ fontSize: 12, color: T.t3, marginTop: 2 }}>The bot joins upcoming Google Calendar events with a video link</div>
          </div>
          <button
            onClick={() => setAutoJoin(!resolvedAutoJoin)}
            style={{
          ...
          >
            <span style={{
              position: 'absolute', top: 3, left: resolvedAutoJoin ? 23 : 3,
        ...
        {resolvedAutoJoin && (
          <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: 14 }}>
            <div style={{ fontSize: 12, color: T.t3, marginBottom: 10, letterSpacing: 0.3 }}>WHICH MEETINGS</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {([
                { value: 'all',      label: 'All meetings', desc: 'Every calendar event with a video link' },
                { value: 'external', label: 'External meetings only', desc: 'Skip internal-only events (no outside attendees)' },
              ] as const).map(opt => (
                <label
                  key={opt.value}
                ...
                >
                  <input
                    ...
                    onChange={() => setJoinType(opt.value)}
        ...
        <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 10 }}>
          <button
            ...
            style={{
              padding: '8px 20px', background: T.grad, border: 'none',
              borderRadius: R.md, color: '#fff', fontSize: 13, fontWeight: 600,
              cursor: settingsSaving ? 'default' : 'pointer', opacity: settingsSaving ? 0.7 : 1,
            }}
          >
