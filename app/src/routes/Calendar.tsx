...
function MeetingBlock({ meeting, onClick }: { meeting: UpcomingMeeting; onClick: () => void }) {
  ...
  return (
    ...
    >
      <div style={{ fontSize: 12, fontWeight: 600, color: T.t1, lineHeight: 1.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {meeting.title}
      </div>
      {height > 38 && (
      ...
      {height > 52 && hasBot && (
        <div style={{ fontSize: 10, fontFamily: T.mono, color: botDone ? T.green : T.sky, marginTop: 4 }}>
          {botDone ? '✓ transcript ready' : `bot ${meeting.bot!.status}`}
        </div>
      )}
    </div>
  );
...
export default function CalendarPage() {
  ...
  const { data, loading } = useApi<{ connected: boolean; meetings: UpcomingMeeting[] }>('/api/meetings/upcoming');
  const meetings = (data?.meetings || []).filter(m => {
    const d = new Date(m.starts_at); d.setHours(0, 0, 0, 0);
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      <div style={{ marginBottom: 6, fontFamily: T.mono, fontSize: 10, letterSpacing: 0.8, color: T.t3 }}>CALENDAR</div>
      <h1 style={{ fontFamily: T.display, fontWeight: 700, fontSize: 26, letterSpacing: -0.5, margin: '0 0 20px', color: T.t1 }}>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 24 }}>
        {weekDays.map(d => {
          ...
          const hasMeetings = (data?.meetings || []).some(m => {
            const md = new Date(m.starts_at); md.setHours(0, 0, 0, 0);
          ...
          return (
            ...
            >
              <span style={{ fontSize: 10, fontFamily: T.mono, color: active ? 'rgba(255,255,255,0.7)' : T.t3, letterSpacing: 0.3 }}>
                {d.toLocaleDateString([], { weekday: 'short' }).toUpperCase()}
              </span>
              <span style={{ fontSize: 16, fontWeight: 700, color: active ? '#fff' : T.t1 }}>
                {d.getDate()}
              </span>
              {hasMeetings && <div style={{ width: 4, height: 4, borderRadius: '50%', background: active ? 'rgba(255,255,255,0.7)' : T.coral }} />}
            </button>
          );
        })}
      </div>
      ...
      {loading ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: T.t3, fontSize: 13 }}>
          <div className="skel" style={{ width: 80, height: 4, borderRadius: 2 }} /> Loading meetings…
        </div>
      ) : !data?.connected ? (
        ...
        <div style={{ display: 'flex', gap: 0 }}>
          {/* Time axis */}
          <div style={{ width: 52, flexShrink: 0, position: 'relative', height: HOUR_HEIGHT * HOURS.length }}>
            {HOURS.map(h => (
              <div
                key={h}
                style={{
          ...
          >
            ...
            {isToday && nowHour >= START_HOUR && nowHour < END_HOUR && (
              ...
              >
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: T.coral, position: 'absolute', left: -4, top: -3 }} />
              </div>
            )}
