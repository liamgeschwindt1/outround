      ...
      >
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <Button
            ...
            onClick={() => {
              navigate(`/practice?meeting=${m.id}`);
            }}
          >
            Get ready
          </Button>
      ...
      <div style={gridStyle}>
        ...
        <div style={{ gridColumn: 'span 8', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <Card>
            ...
            {data.open_next_steps.length > 0 && (
              <>
                ...
                <ul
                  style={{ margin: 0, paddingLeft: 18, color: T.t1, fontSize: 13, lineHeight: 1.6 }}
                >
                  {data.open_next_steps.map((s, i) => (
                    <li key={i}>{s}</li>
                  ))}
                </ul>
              </>
          ...
          <Card>
            <CardHead
              kicker="CRM NOTES"
              title="What your team has logged"
              right={<Tag kind="neutral">{data.notes.length}</Tag>}
            />
            {data.notes.length === 0 ? (
              <EmptyState
                title="No notes yet"
                body="Nothing has been logged for this prospect in Pipedrive."
              />
            ) : (
      ...
      <div style={avatarStyle}>
        {p?.photo_url ? (
          <img
            src={p.photo_url}
            alt={name}
            style={{ width: '100%', height: '100%', borderRadius: R.lg, objectFit: 'cover' }}
          />
        ) : (
          <span style={{ fontFamily: T.display, fontWeight: 600, fontSize: 22, color: T.t2 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
          ...
          {p?.linkedin && (
            ...
            >
              <Tag kind="sky">LinkedIn ↗</Tag>
            </a>
          )}
          {p?.open_deals_count != null && (
            <Tag kind="neutral">
              {String(p.open_deals_count)} open · {String(p.closed_deals_count ?? 0)} closed
            </Tag>
          )}
        </div>
      </div>
  ...
  return (
    <Card>
      <CardHead
        ...
        right={
          deal.status ? (
            <Tag kind={deal.status === 'open' ? 'sky' : 'neutral'}>{deal.status.toUpperCase()}</Tag>
          ) : null
        }
      />
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        ...
        <Stat
          label="Days in stage"
          value={deal.days_in_stage != null ? String(deal.days_in_stage) : '—'}
        />
        <Stat
          label="Probability"
    ...
    <Card>
      ...
      {notes.length === 0 ? (
        <div style={{ fontSize: 13, color: T.t2 }}>
          No coaching notes yet — go a round and your coach will start spotting patterns.
        </div>
      ) : (
...
function PersonaCard({
  ...
  const level = Math.max(1, Math.min(5, persona.resistance_level));
  return (
    <Card>
      <CardHead
        kicker="PERSONA"
        ...
        right={
          <Tag kind={level >= 4 ? 'bad' : level === 3 ? 'warn' : 'good'}>RESISTANCE {level}/5</Tag>
        }
      />
      <div style={{ fontSize: 13, color: T.t1, lineHeight: 1.55, marginBottom: 12 }}>
      ...
      >
        {persona.known_objections.map((o, i) => (
          <li key={i}>
            <span style={{ color: T.t2 }}>“{o}”</span>
          </li>
        ))}
      </ul>
      ...
      <ul style={{ margin: 0, paddingLeft: 18, color: T.t1, fontSize: 13, lineHeight: 1.55 }}>
        {persona.what_moves_them.map((o, i) => (
          <li key={i}>{o}</li>
        ))}
      </ul>
    </Card>
