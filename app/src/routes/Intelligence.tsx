      ...
      {[0, 1, 2].map(i => (
        <div
          ...
          style={{
            ...
            inset: -(i * 20),
            borderRadius: '50%',
            border: `1px solid rgba(240,90,50,${0.12 - i * 0.03})`,
            animation: active ? `orbPulse ${1.2 + i * 0.3}s ease-in-out infinite ${i * 0.2}s` : 'none',
          }}
        />
      ))}
      {/* Core */}
...
export default function IntelligencePage() {
  ...
  const send = async () => {
    ...
    try {
      ...
      const res = await api.post<{ answer: string }>('/api/intel/ask', { question: q }).catch(() => null);
      const answer = res?.answer || 'Intelligence API not yet connected. When your meeting bot has processed calls, Outround will answer questions about your deals, contacts, and patterns here.';
  ...
  return (
    <div style={{ maxWidth: 700, margin: '0 auto', display: 'flex', flexDirection: 'column', height: 'calc(100vh - 120px)' }}>
      <div style={{ marginBottom: 6, fontFamily: "'JetBrains Mono', monospace", fontSize: 10, letterSpacing: 0.8, color: T.t3 }}>INTELLIGENCE</div>
      <h1 style={{ fontFamily: T.display, fontWeight: 700, fontSize: 26, letterSpacing: -0.5, margin: '0 0 4px', color: T.t1 }}>Ask your memory</h1>
      <div style={{ fontSize: 13, color: T.t3, marginBottom: 24 }}>Query your deal history, contacts, and conversation patterns.</div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {messages.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center', maxWidth: 480 }}>
                {[
                  ...
                  <button
                    ...
                    onClick={() => { setInput(q); }}
                    style={{
                      padding: '6px 12px', borderRadius: R.md, background: T.bgElevate,
                      border: `1px solid ${T.borderMd}`, color: T.t2, fontSize: 12, cursor: 'pointer',
                    }}
                  >
                    {q}
                  </button>
          ...
          <div style={{ flex: 1, overflow: 'auto', paddingBottom: 16 }}>
            {messages.map((m, i) => (
              <div key={i} style={{
                ...
                <div style={{
                  maxWidth: '80%',
                  padding: '10px 14px',
                  borderRadius: m.role === 'user' ? `${R.xl}px ${R.xl}px 4px ${R.xl}px` : `${R.xl}px ${R.xl}px ${R.xl}px 4px`,
            ...
            {thinking && (
              <div style={{ display: 'flex', gap: 5, padding: '10px 14px', background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: R.xl, width: 'fit-content' }}>
                {[0, 1, 2].map(i => (
                  <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: T.coral, animation: `bounce 1s ease-in-out ${i * 0.15}s infinite` }} />
                ))}
              </div>
            )}
            <div ref={bottomRef} />
