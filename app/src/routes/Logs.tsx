    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
        ...
        <div style={{ flexShrink: 0, width: 130 }}>
          <span style={{ fontSize: 10, fontFamily: "'JetBrains Mono', monospace", color: T.t4 }}>{dateStr} </span>
          <span style={{ fontSize: 10, fontFamily: "'JetBrains Mono', monospace", color: T.t3 }}>{timeStr}</span>
        </div>
        ...
        {/* Tag */}
        <span style={{ flexShrink: 0, fontSize: 10, fontFamily: "'JetBrains Mono', monospace", color: T.sky, minWidth: 60 }}>
          [{entry.tag}]
        </span>

        {/* Message */}
        <span style={{ flex: 1, fontSize: 12, color: entry.level === 'error' ? T.red : T.t1, fontFamily: 'DM Sans, sans-serif', lineHeight: 1.4 }}>
          {entry.message}
        </span>
...
export default function LogsPage() {
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto' }}>
      <div style={{ marginBottom: 6, fontFamily: "'JetBrains Mono', monospace", fontSize: 10, letterSpacing: 0.8, color: T.t3 }}>LOGS</div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <h1 style={{ fontFamily: T.display, fontWeight: 700, fontSize: 26, letterSpacing: -0.5, margin: 0, color: T.t1 }}>Activity log</h1>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button
            onClick={fetchLogs}
            style={{ padding: '6px 12px', background: T.bgElevate, border: `1px solid ${T.borderMd}`, borderRadius: R.md, color: T.t2, fontSize: 12, cursor: 'pointer' }}
          >
            Refresh
          </button>
          <button
            ...
            style={{
              ...
              borderRadius: R.md, color: autoRefresh ? T.green : T.t2, fontSize: 12, cursor: 'pointer',
            }}
          >
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap' }}>
        {(['all', 'info', 'success', 'warn', 'error'] as const).map(l => (
          ...
          >
            {l.toUpperCase()}{l !== 'all' && counts[l] ? ` (${counts[l]})` : ''}
          </button>
        ))}
        <input
          value={search}
          ...
          style={{
            ...
            padding: '0 10px', color: T.t1, fontSize: 12, outline: 'none', width: 160,
          }}
        />
      </div>
      ...
      <div style={{ background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: R.xl, overflow: 'hidden' }}>
        {loading && (
          <div style={{ padding: 32, textAlign: 'center', color: T.t3, fontSize: 13 }}>Loading logs…</div>
        )}
        {!loading && visible.length === 0 && (
          <div style={{ padding: 32, textAlign: 'center', color: T.t3, fontSize: 13 }}>No log entries match your filter.</div>
        )}
      ...
      {!loading && visible.length > 0 && (
        <div style={{ marginTop: 8, fontSize: 11, color: T.t4, fontFamily: "'JetBrains Mono', monospace", textAlign: 'right' }}>
          {visible.length} entries {logs.length !== visible.length ? `(${logs.length} total)` : ''} · updates every 3s when live
        </div>
      )}
    </div>
  );
