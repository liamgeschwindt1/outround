  ...
  return (
    <span
      style={{
        fontSize: 10,
        fontFamily: "'JetBrains Mono', monospace",
        color: colors[status] ?? T.t3,
        background: `${colors[status] ?? T.t3}18`,
        padding: '2px 7px',
        borderRadius: R.sm,
        border: `1px solid ${colors[status] ?? T.t3}33`,
      }}
    >
      {status}
    </span>
...
export default function CRMPage() {
  ...
  const {
    ...
    loading: cLoading,
    error: cError,
  } = useApi<{ contacts: Contact[]; total: number }>(
    tab === 'contacts' ? '/api/crm/contacts' : null
  );
  const {
    data: dealsData,
    loading: dLoading,
        ...
        <>
          ...
          >
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {['DEAL', 'CONTACT', 'STAGE', 'VALUE', 'CLOSE DATE', 'OWNER', 'STATUS'].map(
                    (h) => (
                      <th key={h} style={{ ...head, textAlign: 'left' }}>
                        {h}
                      </th>
                    )
                  )}
              ...
              <tbody>
                {dealsData.deals.length === 0 && (
                  <tr>
                    <td colSpan={7} style={{ ...cell, textAlign: 'center', color: T.t3 }}>
                      No open deals.
                    </td>
                  </tr>
                )}
                {dealsData.deals.map((d) => (
                  ...
                  >
                    <td style={{ ...cell, color: T.t1, fontWeight: 500, maxWidth: 220 }}>
                      ...
                      {d.org_name && (
                        <div style={{ fontSize: 11, color: T.t3, marginTop: 2 }}>{d.org_name}</div>
                      )}
                    </td>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {contactsData.contacts.length === 0 && (
                  <tr>
                    <td colSpan={5} style={{ ...cell, textAlign: 'center', color: T.t3 }}>
                      No contacts.
                    </td>
                  </tr>
                )}
                {contactsData.contacts.map((c) => (
                  ...
                  >
                    <td style={{ ...cell, color: T.t1, fontWeight: 500 }}>
                      {c.name}
                      {c.phone && (
                        <div style={{ fontSize: 11, color: T.t3, marginTop: 2 }}>{c.phone}</div>
                      )}
                    </td>
