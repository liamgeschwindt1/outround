  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
        ...
        <span style={{ fontSize: 12, fontFamily: T.numeric, color: scoreColor(score) }}>{score}</span>
      </div>
      <div style={{ height: 4, background: T.bgHover, borderRadius: 2 }}>
        <div style={{ height: '100%', width: `${score}%`, background: scoreColor(score), borderRadius: 2, transition: 'width 600ms ease' }} />
      </div>
    </div>
  );
}
...
const SEED_SCORES = [58, 61, 60, 63, 65, 67, 64, 68, 71, 69, 72, 74, 71, 73, 67, 70, 72, 75, 73, 76];

const SEED_HISTORY: (SessionHistoryItem & { persona_name?: string })[] = [
  { id: '1', score: 67, created_at: new Date(Date.now() - 1 * 86400000).toISOString(), persona_id: 'hendrik', persona_name: 'Hendrik', mode: 'Cold Call', duration_seconds: 187, status: 'completed' },
  { id: '2', score: 71, created_at: new Date(Date.now() - 2 * 86400000).toISOString(), persona_id: 'natalie', persona_name: 'Natalie', mode: 'Discovery', duration_seconds: 203, status: 'completed' },
  { id: '3', score: 63, created_at: new Date(Date.now() - 3 * 86400000).toISOString(), persona_id: 'hendrik', persona_name: 'Hendrik', mode: 'Cold Call', duration_seconds: 154, status: 'completed' },
  { id: '4', score: 70, created_at: new Date(Date.now() - 5 * 86400000).toISOString(), persona_id: 'hendrik', persona_name: 'Hendrik', mode: 'Cold Call', duration_seconds: 211, status: 'completed' },
  { id: '5', score: 65, created_at: new Date(Date.now() - 7 * 86400000).toISOString(), persona_id: 'hendrik', persona_name: 'Hendrik', mode: 'Cold Call', duration_seconds: 178, status: 'completed' },
];
...
export default function Progress() {
  ...
  const avgScore = stats?.avg_score ? Math.round(stats.avg_score) : 67;
  const elo = 1847;
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: 14 }}>
          ...
          <Card span={4}>
            ...
            <Num style={{ fontSize: 44, fontWeight: 700, lineHeight: 1, background: T.grad, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', display: 'block' }}>
              {elo.toLocaleString()}
            </Num>
          ...
          {/* ── Score trend ── */}
          <Card span={12}>
            <CardHead kicker="Score Trend" title="" right={<span style={{ fontSize: 12, color: T.t3 }}>last 30 days</span>} />
              ...
              <thead>
                <tr>
                  {['Date', 'Persona', 'Mode', 'Score', 'Delta'].map(h => (
                    <th key={h} style={{ textAlign: 'left', padding: '0 12px 10px 0', fontSize: 10, fontFamily: T.mono, color: T.t4, letterSpacing: 0.5, fontWeight: 400 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {history.map((row, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: '10px 12px 10px 0', color: T.t1 }}>{(row as { persona_name?: string }).persona_name ?? 'Hendrik'}</td>
                      <td style={{ padding: '10px 12px 10px 0', color: T.t2 }}>{row.mode ?? 'Cold Call'}</td>
                      <td style={{ padding: '10px 12px 10px 0' }}>
                        <span style={{ color: scoreColor(row.score ?? 0), fontFamily: T.numeric, fontWeight: 600 }}>{row.score}</span>
                      </td>
